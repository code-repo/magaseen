...
class c_ProductOperation
{
	...
	function m_DeletePublicationImage()
	{
	global $attributes;
	...
	unlink($smallImgPath.$attributes['name']);
	unlink($$thumbImgPath.$attributes['name']);
	unlink($orgImgPath.$attributes['name']);
	$this->dbHandler->m_deletePublicationImageData($attributes['id']);
	...
	{						
		...
		$rsData = $this->dbHandler->m_GetPublications(base64_decode($attributes['m_id']),1);	
		$arData = array();
		$i = 0;
		while(!$rsData->EOF)
		{				
		...
		$rsResult= $this->dbHandler->m_GetRecordDetail('tb_client', $_SESSION['client']['iId']);
		$this->smarty->assign("AVAILABLE_SLOTS", $rsResult->fields['iTotalSlots'] - $rsResult->fields['iConsumedSlots']);
	...
	{						
		...
		if(isset($_FILES['pub_file']['name']) && trim($_FILES['pub_file']['name']) != '')
		{
			...
			$a=time();
			$fileName = "publication_".$a.'.'.$ext;
			$tempfiles = glob($orgImgPath."temp\\*"); // get all file names
foreach($tempfiles as $tempfile){ // iterate files
  if(is_file($tempfile))
    unlink($tempfile); // delete file
}
			...
			if($ext == 'pdf')
			{
				...
				$explode = $explode[0];
				exec("convert  -density 100 -quality 100  -sharpen 0x1.0 ".$file." ".$orgImgPath."temp\\".$a.".png");
				//exec("convert  -density 100  -quality 100 -sharpen 0x1.0 ".$file."  ".$a.".png");
				//exec("convert -verbose -density 150 -trim ".$file." -quality 100 -sharpen 0x1.0 ".$orgImgPath.$a.".png");
				for($i=0;$i<$pages;$i++)
				{
				    $newFile = $explode.$i.'.png';
				    rename($orgImgPath."temp\\".$a."-".$i.".png",$orgImgPath.$newFile);
					$im = new Imagick($orgImgPath.$newFile); 
					...
					//$im->writeImage($orgImgPath.$newFile);
					$im->writeImage($smallImgPath.$newFile);
					$im->adaptiveResizeImage(150, 106);
					$im->writeImage($thumbImgPath.$newFile);
					//echo $explode.$i.'.png';
					$im->clear(); 
					$im->destroy();
					//unlink($orgImgPath.$a."-".$i.".png");
   ...
   $a=time();
     if ($zip->open($file) === true) {
          ...
          for($i = 0; $i < $zip->numFiles; $i++) {
             $entry = $zip->getNameIndex($i);
               if(preg_match('#\.(png)$#i', $entry) || preg_match('#\.(jpg)$#i', $entry))
                {
				$b++;
                ...
                copy('zip://'.$orgImgPath.$fileName.'#'.$entry, $orgImgPath.'temp\\'.$fileName.$b.'.png');
                //$content = file_get_contents("images\\".$value."png");
				$newFile = $explode.$i.'.png';
                rename($orgImgPath.'temp\\'.$fileName.$b.'.png',$orgImgPath.$newFile);
                $im = new Imagick($orgImgPath.$newFile); 
	...
	{	
		...
		$rsApp = $this->dbHandler->m_GetRecords('tb_article', 'iClientId='.$_SESSION['client']['iId'].' AND iPublicationId='.$attributes['pub_id']);	
	...
	{						
		...
		if(isset($attributes['id']) && $attributes['id'] > 0)
		{
			$rsResult= $this->dbHandler->m_DeletePublication('tb_publication', $attributes['id']);
	...
	{						
		...
		if(isset($attributes['id']) && $attributes['id'] > 0)
		{
			$rsPubFile = $this->dbHandler->m_GetRecords('tb_publication_files', 'iPublicationId='.$attributes['id']);
			while(!$rsPubFile->EOF)
			{
				...
				while(!$rsHotspot->EOF)
				{
					...
					{
						@unlink(SITE_PATH.'product/hotspot/'.$rsHotspotFile->fields['vFile']);
						@unlink(SITE_PATH.'product/hotspot/thumb/'.$rsHotspotFile->fields['vFile']);
						$this->dbHandler->m_DeleteRecord('tb_hotspot_files', $rsHotspotFile->fields['iId']);
						$rsHotspotFile->MoveNext();
					}
					$this->dbHandler->m_DeleteRecord('tb_hotspots', $rsHotspot->fields['iId']);
					$rsHotspot->MoveNext();
				}
				@unlink(SITE_PATH.'product/publication/'.$rsPubFile->fields['vFile']);
				@unlink(SITE_PATH.'product/publication/thumb/'.$rsPubFile->fields['vFile']);
				@unlink(SITE_PATH.'product/publication/small/'.$rsPubFile->fields['vFile']);
				$this->dbHandler->m_DeleteRecord('tb_publication_files', $rsPubFile->fields['iId']);
				$rsPubFile->MoveNext();
			}
	...
	{						
		...
		$arData['iPublicationId'] = $attributes['pub_id'];
	...
	{	
		...
		while(!$rsMagazine->EOF)
		{
			...
			$rsPub = $this->dbHandler->m_GetPublications($rsMagazine->fields['iId'],2);
			$j=1;
			if($rsPub){
			while(!$rsPub->EOF)
			{
	...
	{	
		...
		while(!$rsPubFile->EOF)
		{
			...
			// $rsHotspot = $this->dbHandler->m_GetRecords('tb_hotspots', 'iPubFileId='.$rsPubFile->fields['iId']);
			$rsHotspot = $this->dbHandler->m_GetHotspots($rsPubFile->fields['iId']);
			if($rsHotspot){
			while(!$rsHotspot->EOF)
			{
				if(!$rsHotspot->fields['file_cnt']){
					$rsHotspot->MoveNext();
					continue;
				}
				$xml .= '<object ';
				...
				while(!$rsHotspotFile->EOF)
				{
					// print_r($rsHotspotFile->fields);
					$xml .= '<object ';
					$xml .= 'object_type="Hotspot_Data" ';
					$xml .= 'Name="'.addslashes($rsHotspotFile->fields['vName']).'" ';
					$xml .= 'Description="'.addslashes($rsHotspotFile->fields['vDesc']).'" ';
					$xml .= 'Link="'.addslashes($rsHotspotFile->fields['vLink']).'" ';
					if($rsHotspotFile->fields['vFile'])
					$xml .= 'File="'.SITE_URL.'product/hotspot/'.$rsHotspotFile->fields['vFile'].'" ';
					$xml .= 'Id="'.$rsHotspotFile->fields['iId'].'" ';
