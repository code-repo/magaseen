...
class c_Captcha
{
	...
	function m_GenImage($num){
		...
		session_start();		
		$_SESSION['sesCaptchaCode'] = $this->texto;
	}
...
//		$possible="AHLFTDSKNPRVY24579";
	...
	{					
		global $_CONF;		
		$this->im=imagecreatefromjpeg($_CONF['SiteUploadImagePath']."libimages/captcha_image.jpg");
		$ident = 15;
		$black = imagecolorallocate($this->im, 0, 0, 0);

		for ($i=0;$i<$this->numChars;$i++){
			$char = substr($this->texto, $i, 1);			
			$font = $_CONF['SiteUploadImagePath']."libimages/fonts/verdana.ttf";			
			$y = round(($this->h-10)/2);
			$t = rand(2,20);
			$size=rand(15,20);
	...
	function m_CheckCode($stEnteredCode)
	{
		...
		if ($stEnteredCode == $_SESSION['sesCaptchaCode'])
		{
			return true;
		}
		else
