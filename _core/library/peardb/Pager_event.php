...
class DB_Pager extends PEAR
{
    ...
    */
	...
	// Added By RPK on 060119
    {
		...
		// Added By RPK on 060119		
		//$this->smarty = new Smarty($GLOBALS['sPath'].'/templates');		
		...
		$inPagePosition = strripos($_SERVER[REQUEST_URI],"/page/");
		$stDomainName = "http://".$_SERVER[HTTP_HOST]."/";
		//echo $GLOBALS['_CONF']['EventUrl'];
		if(($stDomainName == $GLOBALS['_CONF']['EventUrl'] ||  $stDomainName == $GLOBALS['_CONF']['PlaceUrl']
		||  $stDomainName == $GLOBALS['_CONF']['PeopleUrl'])
		&& $_SERVER[REQUEST_URI] == "/")
		{
			$stSpecialVar = "list";
		}
		if($inPagePosition)
		{
			$stPagingParam = substr($_SERVER[REQUEST_URI], 0, $inPagePosition); 
		}
		else
		{
			 $stPagingParam = $_SERVER[REQUEST_URI].$stSpecialVar;
		}		
		$this->smarty->assign('PAGINGURL',"http://".$_SERVER[HTTP_HOST].$stPagingParam);		
