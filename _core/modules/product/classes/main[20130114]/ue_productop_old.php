...
class c_ProductOperation
{
	...
	{	
		...
		$mId = base64_decode($attributes['m_id']);
		
		$rsApp = $this->dbHandler->m_GetRecords('tb_publication', 'iMagazineId='.$mId);	
		$i=0;
		while(!$rsApp->EOF)
		{
			$arData[$i] = $rsApp->fields;
			$arData[$i]['iId'] = base64_encode($rsApp->fields['iId']);
			$i++;
			$rsApp->MoveNext();
		}
	...
	{						
		...
		{			
			...
			elseif($attributes['type'] == 'gallery')
			{
				...
				print $rsResult= $this->dbHandler->m_UpdateGalleryRecords($arData, $id);
				die;
			}
			elseif($attributes['type'] == 'link')
			{
				$id = $attributes['link_id'];
				$arData['vName'] = $attributes['link_url'];
			}
		...
		if(isset($_FILES['pub_file']['name']) && trim($_FILES['pub_file']['name']) != '')
		{
			...
			if($ext == 'pdf')
			{
				...
				for($i=0;$i<$pages;$i++)
				{
					...
					$id= $this->dbHandler->m_AddPublicationImage($newFile, $dispName, $attributes['pub_id']);
				...
				move_uploaded_file($source, $orgImgPath.$fileName);
				$id= $this->dbHandler->m_AddPublicationImage($fileName, $dispName, $attributes['pub_id']);
				$result[0]['id'] = $id;
	...
	{	
		...
		while(!$rsApp->EOF)
		{
			$html .= '<li style="cursor:pointer;" id="file_'.$rsApp->fields['iId'].'" onclick="set_image(this.id);">'.$rsApp->fields['vDispName'].'</li>';
			// $html .= '<div id="src_file_'.$rsApp->fields['iId'].'" style="display:none;"><img src="'.SITE_URL.'product/publication/small/'.$rsApp->fields['vFile'].'" /></div>';
			$rsApp->MoveNext();
		}
		print $html;
		die;
	...
	{	
		...
		while(!$rsPubFile->EOF)
		{
			...
			<object Id="10" Name ="Inroduction" ObjectiveType ="Article" section_id="1" obj_ParentId="1"></object>';
			$xml .='</object>';
	...
	{	
		global $attributes;
		$xml = '<objects>';
		$rsmag = $this->dbHandler->m_GetRecordDetail('tb_magazine', $attributes['mag_id']);
		if($rsmag)
		{
			while(!$rsmag->EOF)
			{
