...
class c_ProductDb
{
	...
	function m_Gethotspotname($hotspotid)
	{
		$stQuery="select * from tb_publication_files where iPublicationId='".$hotspotid."'";
		$rsResult = $this->obDbase->Execute($stQuery);
		if (!$rsResult)
		{
			return false;
	...
	function m_GetRecordDetail($table)
	{						
		$stQuery = "SELECT * FROM $table";		
		$rsResult = $this->obDbase->Execute($stQuery);
		if (!$rsResult)
		{        				
			return false;
	...
	//amit start
	function m_GetMagzaineRecord($table,$mag_id)
	{						
		$stQuery = "SELECT * FROM $table where iId=$mag_id";		
		$rsResult = $this->obDbase->Execute($stQuery);
		if (!$rsResult)
		{        				
			return false;
	...
	//amit sponsor start
	function m_GetSponsor($Pubfileid)
	{
		$stQuery = "select * from tb_hotspots where iPubfieldId=$Pubfileid";
