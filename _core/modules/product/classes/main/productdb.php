...
class c_ProductDb
{
	...
	function m_GetHotspots($pub_file_id)
	{						
		$stQuery = "SELECT h. * , (SELECT COUNT( iId ) FROM tb_hotspot_files WHERE h.iId = iHotspotId) as file_cnt FROM tb_hotspots h WHERE iPubFileId=$pub_file_id";
		$rsResult = $this->obDbase->Execute($stQuery);
		if (!$rsResult)
		{
			return false;
	...
	function m_AddEditHotspotFile($arData, $id = NULL)
	{
		if($id > 0)
		{
		$thumbPath = SITE_PATH.'product/hotspot/thumb/';
		$orgPath = SITE_PATH.'product/hotspot/';					
         $stQuery = "SELECT * FROM tb_hotspot_files WHERE iId=$id";
		 $sql = mysql_query($stQuery);
		while($result = mysql_fetch_assoc($sql))
		{
		unlink($orgPath.$result['vFile']);
		unlink($thumbPath.$result['vFile']);
		}
			$stQuery = "UPDATE tb_hotspot_files 
	...
	function m_deletePublicationImageData($id)
	{
	$stQuery = "DELETE FROM tb_publication_files WHERE iId IN ($id)";
		$rsResult = $this->obDbase->Execute($stQuery);
		$stQuery = "DELETE FROM tb_hotspots WHERE iPubFileId IN ($id)";
		$rsResult = $this->obDbase->Execute($stQuery);
	}
	
	function m_DeletePublication($table, $id)
	{
		...
		$stQuery="SELECT * FROM tb_publication_files WHERE iPublicationId = ".$id;
		$sql = mysql_query($stQuery);
		while($result = mysql_fetch_assoc($sql))
		{
		unlink($orgImgPath.$result['vFile']);
		unlink($thumbImgPath.$result['vFile']);
		unlink($smallImgPath.$result['vFile']);
		}
		$stQuery="SELECT * FROM tb_publication_files WHERE iPublicationId =". $id;
		$sql = mysql_query($stQuery);
		while($result = mysql_fetch_assoc($sql))
		{
		 $stQ="SELECT * FROM tb_hotspots WHERE iPubFileId =".$result['iId'];
		 $sql1 = mysql_query($stQ);
		 while($rs = mysql_fetch_assoc($sql1))
		{
		 $sQ="SELECT * FROM tb_hotspot_files WHERE iHotspotId =".$rs['iId'];
		 $sql2 = mysql_query($sQ);
		 while($r = mysql_fetch_assoc($sql2))
		{
		 if(is_file($thumbPath.$r['vFile']))
		unlink($thumbPath.$r['vFile']);
		unlink($orgPath.$r['vFile']);
		}
		}
		}
		...
		while($result = mysql_fetch_assoc($sql))
		{
		 ...
		 while($rs = mysql_fetch_assoc($sql1))
		{
		 $sQ="DELETE FROM tb_hotspot_files WHERE iHotspotId =".$rs['iId'];
		 $sql2 = mysql_query($sQ);
		 }
		}
		
		$stQuery="DELETE FROM tb_hotspots WHERE iPubFileId IN (SELECT iId FROM tb_hotspots WHERE iPublicationId=".$id.")";
		$sql = mysql_query($stQuery);
		
		 
		 $stQuery="DELETE FROM tb_publication_files WHERE iPublicationId = ".$id;
		$sql = mysql_query($stQuery);
		 
	}
	
	function m_DeleteHotspot($table, $id)
	{
	    $thumbPath = SITE_PATH.'product/hotspot/thumb/';
		$orgPath = SITE_PATH.'product/hotspot/';
		//$stQuery = "DELETE FROM $table WHERE iId IN ($id)";
		$sQ="SELECT * FROM tb_hotspot_files WHERE iHotspotId =".$id;
		 $sql2 = mysql_query($sQ);
		 while($r = mysql_fetch_assoc($sql2))
		{
		 if(is_file($thumbPath.$r['vFile']))
		...
		$stQuery = "DELETE FROM tb_hotspot_files WHERE iHotspotId=".$id;
		$sql = mysql_query($stQuery);
		$stQuery = "DELETE FROM $table WHERE iId IN ($id)";
		$sql = mysql_query($stQuery);
	...
	function m_GetPublications($id,$function)
	{	if($function==1)
	    {
		$stQuery = "SELECT p.*, 
		...
		$stQuery = "SELECT p.*, 
		(SELECT COUNT(iId) FROM tb_publication_files WHERE iPublicationId=p.iId) AS total_pages, 
		(SELECT vFile FROM tb_publication_files WHERE iPublicationId=p.iId ORDER BY iId LIMIT 1) AS vFile FROM tb_publication p WHERE p.iMagazineId=".$id." AND p.iPublished=1 AND p.iId IN (SELECT iPublicationId FROM  tb_publication_files)ORDER BY p.iId DESC";
		}
		$rsResult = $this->obDbase->Execute($stQuery);
		if (!$rsResult)
		{
