...
class c_ProductOperation
{
	...
	{						
		...
		if(isset($_FILES['pub_file']['name']) && trim($_FILES['pub_file']['name']) != '')
		{
			...
			if($ext == 'pdf')
			{
				...
				$explode = $explode[0];
				exec("convert  -density 100 -quality 100  -sharpen 0x1.0 ".$file." ".$orgImgPath.$a.".png");
				...
				for($i=0;$i<$pages;$i++)
				{
					$im = new Imagick($orgImgPath.$a."-".$i.".png"); 
   ...
   $a=time();
	...
	{						
		...
		$this->dbHandler->m_DeleteHotspot('tb_hotspots', $id);
	...
	{	
		...
		while(!$rsMagazine->EOF)
		{
			...
			while(!$rsPub->EOF)
			{
				...
				$xml .= 'Downloadurl="'.SITE_URL.'product/publication/small/'.$rsPub->fields['vFile'].'" ';
				$xml .= 'id="'.$rsPub->fields['iId'].'" ';
				$xml .= 'parentid="'.$rsPub->fields['iMagazineId'].'" ';
				$xml .= '>';
				$xml .= '</object>';
